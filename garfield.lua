...
local GarfieldFolder = "garfield"
local ConfigurationFolder = GarfieldFolder.."/Configurations"
local ConfigurationExtension = ".gfld"



local GarfieldLibrary = {
	Flags = {},
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across Garfield
		...
		Light = {
			TextFont = "Gotham", -- Default will use the various font faces used across Garfield
...
local Garfield = game:GetObjects("rbxassetid://10804731440")[1]



if gethui then
	Garfield.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(Garfield)
	Garfield.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	Garfield.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	Garfield.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Garfield.Name and Interface ~= Garfield then
			Interface.Enabled = false
			Interface.Name = "Garfield-Old"
		end
	end
else
...
local Main = Garfield.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

Garfield.DisplayOrder = 100
...
local Notifications = Garfield.Notifications

local SelectedTheme = GarfieldLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = GarfieldLibrary.Theme[ThemeName]
	for _, obj in ipairs(Garfield:GetDescendants()) do
	...
	Garfield.Main.BackgroundColor3 = SelectedTheme.Background
	Garfield.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Garfield.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Garfield.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow
	
	Garfield.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Garfield.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Garfield.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	table.foreach(Data, function(FlagName, FlagValue)
		if GarfieldLibrary.Flags[FlagName] then
			spawn(function() 
				if GarfieldLibrary.Flags[FlagName].Type == "Colorpicker" then
					GarfieldLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if GarfieldLibrary.Flags[FlagName].CurrentValue or GarfieldLibrary.Flags[FlagName].CurrentKeybind or GarfieldLibrary.Flags[FlagName].CurrentOption ~= FlagValue then GarfieldLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			GarfieldLibrary:Notify({Title = "Flag Error", Content = "Garfield was unable to find '"..FlagName.. "'' in the current script"})
		end
	end)
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(GarfieldLibrary.Flags) do
...
function GarfieldLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= GarfieldLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("Garfield | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
		...
		if Garfield.Name == "Garfield" then
...
function Hide()
	Debounce = true
	GarfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping RightShift", Duration = 7})
...
function GarfieldLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Garfield Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "Garfield Interface Suite" then
		LoadingFrame.Version.Text = "Garfield UI"
	end
	...
	pcall(function()
		...
		if not isfolder(GarfieldFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(GarfieldFolder.."/Discord Invites") then
			makefolder(GarfieldFolder.."/Discord Invites")
		end
		if not isfile(GarfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(GarfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Garfield RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(GarfieldFolder.."/Key System") then
			makefolder(GarfieldFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			...
			if not Success then
				print("Garfield | "..Settings.KeySettings.Key.." Error " ..tostring(Response))
			end
		end
		...
		if isfile(GarfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			if readfile(GarfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) == Settings.KeySettings.Key then
				Passthrough = true
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				if KeyMain.Input.InputBox.Text == Settings.KeySettings.Key then
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(GarfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, Settings.KeySettings.Key)
						end
						GarfieldLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Garfield | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Garfield | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					if Dropdown.Selected.Text ~= Option then
						...
						if not Success then
							...
							print("Garfield | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					GarfieldLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Garfield | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					GarfieldLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Garfield | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					GarfieldLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Garfield | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					GarfieldLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function GarfieldLibrary:Destroy()
	Garfield:Destroy()
end
...
function GarfieldLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				GarfieldLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

return GarfieldLibrary
